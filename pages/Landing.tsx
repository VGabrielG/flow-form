    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl w-full z-10 text-center">
        ...
        <div className="mt-20 flex justify-center space-x-6">
           <button onClick={() => setLanguage('en')} className={`px-6 py-2 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${language === 'en' ? 'bg-white text-slate-900' : 'text-slate-500 border border-white/10 hover:border-white/30'}`}>English</button>
           <button onClick={() => setLanguage('es')} className={`px-6 py-2 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${language === 'es' ? 'bg-white text-slate-900' : 'text-slate-500 border border-white/10 hover:border-white/30'}`}>Español</button>
        </div>
      </div>
    </div>
  );
